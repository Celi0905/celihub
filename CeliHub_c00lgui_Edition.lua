...
TabSet:CreateInput({Name="🎵 Music ID",PlaceholderText="142930454",RemoveTextAfterFocusLost=false,Callback=function(v) if v~="" then Cfg.MusicID=v end end})
TabSet:CreateSlider({Name="🎵 Music Pitch",Range={0,3},Increment=0.1,CurrentValue=1,Callback=function(v) Cfg.MusicPitch=v end})
...
TabSky:CreateButton({Name="▶️ Musik starten",Callback=function()
    pcall(function() local old=workspace:FindFirstChild("CeliHubMusic"); if old then old:Destroy() end; local s=Instance.new("Sound",workspace); s.Name="CeliHubMusic"; s.SoundId="rbxassetid://"..Cfg.MusicID; s.PlaybackSpeed=Cfg.MusicPitch; s.Volume=0.6; s.Looped=true; s:Play() end); N("🎵","ID "..Cfg.MusicID,4)
end})
TabSky:CreateButton({Name="⏹️ Musik stoppen",Callback=function()
    pcall(function() local s=workspace:FindFirstChild("CeliHubMusic"); if s then s:Stop(); s:Destroy() end end); N("⏹️","Gestoppt.",2)
end})
TabSky:CreateSection("🎵 Preset Songs")
local SONGS={{"🎵 Electro Sp00k","142930454"},{"🎵 Wonga","130768996"},{"🎵 Chop Suey","143666548"},{"😱 Scream","26120219"},{"🎵 Never Gonna","130860862"}}
for _,m in ipairs(SONGS) do
    local lbl,id=m[1],m[2]
    TabSky:CreateButton({Name=lbl,Callback=function()
        pcall(function() local old=workspace:FindFirstChild("CeliHubMusic"); if old then old:Destroy() end; local s=Instance.new("Sound",workspace); s.Name="CeliHubMusic"; s.SoundId="rbxassetid://"..id; s.PlaybackSpeed=Cfg.MusicPitch; s.Volume=0.6; s.Looped=true; s:Play() end); N("🎵",lbl.." spielt!",3)
    end})
end
